...
<title>.: BlogChat - Archivio: <?php echo $_SESSION["utente"]; ?> :.</title>
...
<div id="bodyPan">
  ...
  <?php
		if(!isset($_GET["pagina"]))
			...
			$pagine = ceil($numero / $config["post_per_pagina"]);
			$contenuto = leggi_post_utente(($pagina -1) + $config["post_per_pagina"], $config["post_per_pagina"], $_SESSION["utente"]);
			...
			if (count($contenuto) > 0){
				...
				foreach ($contenuto as $post){
					...
					echo "<p>Non hai post all'interno del tuo archivio. Per poter vedere dei post all'interno del tuo archivio devi aver scritto almeno 6 post!</p>";
