...
<title>.: BlogChat - Utente: <?php echo $_SESSION["utente"]; ?> :.</title>
...
<div id="bodyPan">
  <div id="bodyleftPan">
  <?php
		//chiamata della funzione leggi presente nel file "funzioni.php"; restituisce in un array il numero di post indicati nel secondo argomento, a partire da quello che nel blog occupa la posizione indicata dal suo primo argomento (0 = primo post)
			$contenuto = leggi_post_utente(0 , $config["post_per_pagina"], $_SESSION["utente"]);
			...
			if (count($contenuto) > 0){
				...
				foreach ($contenuto as $post){
					...
					echo "<p>Non hai post all'interno del blog. Inizia a scrivere i tuoi pensieri e/o opinioni!</p>";
